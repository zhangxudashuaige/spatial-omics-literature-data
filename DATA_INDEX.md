...
这个仓库不是大型数据镜像。GitHub 保存论文记录、真实下载地址、文件清单、校验值和脚本；所有新数据模块统一位于 [`datasets/`](datasets/)，大型表达矩阵、FASTQ、图像和权重保存在各模块中被忽略的 `raw/`、`external/` 或 `processed/` 目录。
...
| GraphSAGE | [`datasets/paper_datasets/`](datasets/paper_datasets/) | PPI ZIP 与官方 toy PPI 已校验 | Reddit 仅部分下载；WoS Citation 需要许可 |
| TABULA | [`datasets/paper_datasets/`](datasets/paper_datasets/) | 无论文精确 15M 训练集 | 精确 Census 版本和完整 cell ID 清单未公开 |
| HEIST | [`datasets/paper_datasets/`](datasets/paper_datasets/) | 只有资源元数据 | 22.3M 预训练集不是统一包；多数精确文件仍待确认 |
...
| scMamba | [`datasets/scmamba/`](datasets/scmamba/) | PBMC.h5mu 已校验 | 其余七套论文精确处理输入未定位/未下载 |
| SpaGCN | [`datasets/spagcn/`](datasets/spagcn/) | 151673 表达矩阵与坐标 | 图像、邻接矩阵及其余论文数据未下载 |
| SToFM | [`datasets/stofm/`](datasets/stofm/) | 无语料文件 | 已保存 1,869 文件、979.49 GB 官方清单；按文件选择下载 |
