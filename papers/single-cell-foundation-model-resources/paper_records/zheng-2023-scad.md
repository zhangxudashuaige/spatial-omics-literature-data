# Enabling Single-Cell Drug Response Annotations from Bulk RNA-Seq Using SCAD

- **论文 ID**：`zheng-2023-scad`
- **作者**：Zetian Zheng et al.
- **年份 / 来源**：2023，Advanced Science
- **DOI**：10.1002/advs.202204113
- **官方论文页**：https://doi.org/10.1002/advs.202204113
- **与数据的关系**：benchmark construction and model paper
- **PDF 保存状态**：未上传；尚未逐篇确认出版商 PDF 的再分发许可

## 关联数据

- `DS034`：SCAD drug-response collection

## 本仓库保存了什么

本页保存论文元数据、官方入口以及论文—数据的对应关系。数据下载信息见 [downstream_datasets.csv](../metadata/downstream_datasets.csv)，引用信息见 [sources.bib](../metadata/sources.bib)。

