# Exploring genetic interaction manifolds constructed from rich single-cell phenotypes

- **论文 ID**：`norman-2019-perturb-seq`
- **作者**：Thomas M. Norman et al.
- **年份 / 来源**：2019，Science
- **DOI**：10.1126/science.aax4438
- **官方论文页**：https://doi.org/10.1126/science.aax4438
- **与数据的关系**：data generation and perturbation atlas
- **PDF 保存状态**：未上传；尚未逐篇确认出版商 PDF 的再分发许可

## 关联数据

- `DS016`：Norman Perturb-seq

## 本仓库保存了什么

本页保存论文元数据、官方入口以及论文—数据的对应关系。数据下载信息见 [downstream_datasets.csv](../metadata/downstream_datasets.csv)，引用信息见 [sources.bib](../metadata/sources.bib)。

